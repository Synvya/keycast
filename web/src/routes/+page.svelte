<script lang="ts">
	...
	const groupedSessions = $derived.by(() => {
		...
		for (const session of sessions) {
			...
			const key = isOAuth
				? session.redirect_origin
				: session.bunker_pubkey;
			...
			if (existing) {
				...
				if (session.last_activity) {
					if (
						!existing.latest_activity ||
						session.last_activity > existing.latest_activity
					) {
	...
	const isWhitelisted = $derived(
		currentUser?.pubkey
			? getAllowedPubkeys().includes(currentUser.pubkey)
			: false,
	);
	...
	async function loadSessions() {
		...
		try {
			const response = await api.get<{ sessions: BunkerSession[] }>(
				"/user/sessions",
			);
	...
	async function revokeGroupedSession(group: GroupedSession) {
		try {
			for (const pubkey of group.bunker_pubkeys) {
				await api.post("/user/sessions/revoke", {
					bunker_pubkey: pubkey,
				});
			}
			...
			try {
				...
				if (response.ok) {
					...
					if (data.authenticated && data.pubkey) {
						const savedMethod =
							(localStorage.getItem("keycast_auth_method") as
								| "nip07"
								| "cookie") || "cookie";
			...
			<section class="identity-section">
				...
				<div class="identity-card">
						...
						<div class="identity-row">
							...
							<div class="identity-info">
								<span class="identity-value"
									>Signed in via NIP-07 extension</span
								>
					...
					<div class="identity-row">
						...
						<div class="identity-info">
							<span
								class="identity-value mono"
								title={formatPubkey(currentUser.pubkey)}
							>
								{formatPubkey(currentUser.pubkey).slice(
									0,
									12,
								)}...{formatPubkey(currentUser.pubkey).slice(
									-8,
					...
					{#if showLearnMore}
						<div class="learn-content">
							<div class="learn-block">
								<h4>
									<Key size={16} weight="fill" /> Your Keys Explained
								</h4>
								<p>
									<strong>Your npub</strong> (public key) is like
									a username. Share it so others can find you across
									any Nostr app.
								</p>
								<p>
									<strong>Your nsec</strong> (private key)
									proves you own this identity. Keep it safe!
									Find it in
							...
							<div class="learn-block">
								<h4>
									<ShieldCheck size={16} weight="fill" /> Where
									Is Your Key?
								</h4>
								<p>
									When you sign up with email and password,
									Synvya generates a Nostr key for you and
									stores it on <a
										href="https://login.divine.video"
										target="_blank"
										rel="noopener noreferrer"
										>login.divine.video</a
									>, encrypted using the same standards banks
									and password managers rely on (<a
										href="https://en.wikipedia.org/wiki/Advanced_Encryption_Standard"
										target="_blank"
										rel="noopener noreferrer">1</a
									>,<a
										href="https://cloud.google.com/security/products/security-key-management"
										target="_blank"
										rel="noopener noreferrer">2</a
									>). Your key is only decrypted in memory
									when an app needs to sign on your behalf,
									and is never stored in plain text.
								</p>
								<p>
									Any Nostr app that supports Synvya Login,
									like <a
										href="https://privdm.com"
										target="_blank"
										rel="noopener noreferrer"
										class="inline-link"
										>Priv DM <ArrowSquareOut size={12} /></a
									>, can use your identity with just your
									email and password. No copying keys between
									apps, no manual setup.
								</p>
							</div>

							<div class="learn-block">
								<h4>
									<Key size={16} weight="fill" /> Already Have
									a Nostr Key?
								</h4>
								<p>
									You don't need a Synvya account at all.
									Import your nsec into the Synvya app and
							...
							<div class="learn-block">
								<h4>
									<Export size={16} weight="fill" /> Want Full
									Control of Your Key?
								</h4>
								<p>
									If you started with email and password but
									want full control, export your nsec from <a
								...
								<ul class="learn-list">
									<li>
										...
										<a
											...
											rel="noopener noreferrer"
											class="inline-link"
											>nsec.app <ArrowSquareOut
												size={12}
											/></a
										> (any browser) turn your device into a personal
										signing server. When a Nostr app needs your
										signature, it asks your device and your key
										never leaves it.
									</li>
									<li>
										...
										or
										<a
											href="https://chromewebstore.google.com/detail/soapboxpub-signer/nnodjkgakfpkckcnbacpcjbpmlmbihdd"
											target="_blank"
											rel="noopener noreferrer"
											class="inline-link"
											>Soapbox Signer <ArrowSquareOut
												size={12}
											/></a
										>
										(Chrome, Firefox) keep your key in the browser
										itself.
										<a
											href="https://apps.apple.com/app/nostash/id6499558903"
											target="_blank"
											rel="noopener noreferrer"
											class="inline-link"
											>Nostash <ArrowSquareOut
												size={12}
											/></a
										> does the same for Safari on iOS.
									</li>
								</ul>
								<p>
									With these options, each app that needs your
									signature must connect to your signer
									individually. Synvya Login handles that for
									you automatically.
								</p>
							</div>
							...
							<div class="learn-block highlight">
								<h4>
									<ShieldCheck size={16} weight="fill" /> Why This
									Matters
								</h4>
								<p>
									Unlike Twitter or Facebook, <strong
										>no company owns your Nostr identity</strong
									>. Even if Synvya disappeared tomorrow, your
									identity and content would still exist on
									the network. Export your key and continue
									anywhere.
								</p>
								<p class="learn-cta">
								...
								<div class="learn-explore">
									<a
										...
										target="_blank"
										rel="noopener noreferrer"
										>Explore Nostr apps at nostrapps.com <ArrowSquareOut
											size={12}
										/></a
									>
								</div>
					...
					{#if groupedSessions.length === 0}
						<div class="empty-state">
							...
							<p class="hint">
								...
								Synvya" already work with your email and
								password. Use this to connect to other Nostr
								apps. Browse them at <a
									href="https://nostrapps.com"
									target="_blank"
									...
									>
										<div class="app-info">
											<p class="app-name">
												...
												{#if group.isOAuth}
													<span
														class="connection-badge oauth"
														>Synvya Login</span
													>
												{:else}
													<span
														class="connection-badge manual"
														>Bunker</span
													>
												{/if}
											</p>
											...
											<div class="details-grid">
												...
												{:else}
													...
													<div class="detail-item">
														...
														<span
															class="detail-value"
															>{session.activity_count}</span
														>
													</div>
													{#if session.client_pubkey}
														<div
															class="detail-item full-width pubkey-row"
														>
															<div
																class="detail-header"
															>
				...
				<section class="teams-section">
					...
					{#if teams.length === 0}
						<div class="empty-state">
							<p>No teams yet.</p>
							<p class="hint">
								Teams let you manage shared Nostr keys with
							...
							{#each teams as team}
								...
								>
									<div class="team-info">
										...
										<p class="team-meta">
											{team.team_users.length} members • {team
												.stored_keys.length} keys
										</p>
									</div>
		...
		>
			...
			<div class="modal" onclick={(e) => e.stopPropagation()}>
				...
				<p class="modal-warning">
					This app will no longer be able to sign events on your
					behalf.
				</p>
				<div class="modal-actions">
					...
					<button
						...
						onclick={() =>
							sessionToRevoke &&
							revokeGroupedSession(sessionToRevoke)}
					>
						Revoke Access
					</button>
				</div>
		...
		<div class="landing-content">
			...
			{#if hasExtension}
				...
				>
					{isNip07Loading
						? "Connecting..."
						: "Admin access with Nostr extension"}
				</button>
			{/if}
			...
			<div class="features-grid">
				<div class="feature-card">
					...
					<p>
						No technical setup. Create an account like you would on
						any other service.
					</p>
				</div>
				...
				<div class="feature-card">
					...
					<p>
						Sign in to any Nostr app that supports Synvya Login. One
						account, everywhere.
					</p>
				</div>
				...
				<div class="feature-card">
					...
					<p>
						Your keys are encrypted and stored safely, like your
						passwords in iCloud or Google.
					</p>
				</div>
			</div>
			...
			<p class="nostr-learn-more">
				New to Nostr? <a
					...
					target="_blank"
					rel="noopener noreferrer">Learn how it works</a
				>
			</p>
		</div>
	</div>
...
<style>
	...
	.status-badge.admin {
		background: color-mix(
			in srgb,
			var(--color-divine-purple, #8b5cf6) 20%,
			transparent
		);
	...
	.learn-explore {
		...
		border-top: 1px solid
			color-mix(in srgb, var(--color-divine-green) 15%, transparent);
	}
	...
	.app-card:hover {
		border-color: color-mix(
			in srgb,
			var(--color-divine-green) 50%,
			var(--color-divine-border)
		);
	}
	...
	.connection-badge.oauth {
		background: color-mix(
			in srgb,
			var(--color-divine-green) 15%,
			transparent
		);
		color: var(--color-divine-green);
		border: 1px solid
			color-mix(in srgb, var(--color-divine-green) 30%, transparent);
	}

	.connection-badge.manual {
		background: color-mix(
			in srgb,
			var(--color-divine-text-tertiary) 10%,
			transparent
		);
		color: var(--color-divine-text-secondary);
		border: 1px solid
			color-mix(
				in srgb,
				var(--color-divine-text-tertiary) 25%,
				transparent
			);
	}
