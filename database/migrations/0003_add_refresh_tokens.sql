...
CREATE INDEX idx_refresh_token_hash ON oauth_refresh_tokens(token_hash)
    WHERE consumed_at IS NULL;

-- Index for finding all tokens for an authorization (for revocation cascade)
CREATE INDEX idx_refresh_token_auth_id ON oauth_refresh_tokens(authorization_id);

-- Index for cleanup of expired/consumed tokens
CREATE INDEX idx_refresh_token_expires ON oauth_refresh_tokens(expires_at)
    WHERE consumed_at IS NULL;
